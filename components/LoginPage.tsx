...
import { Mail, Lock, Loader2, ChevronRight, AlertCircle } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ errorOverride }) => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  // Sync error from parent (authorization check)
  useEffect(() => {
    ...
    <div className="min-h-screen w-full flex overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-orange-500 relative items-center justify-center p-20">
        ...
        <div className="relative z-10 text-white max-w-lg">
          ...
          <h1 className="text-5xl font-bold mb-6 leading-tight">Memory of Touggourt Admin Portal</h1>
      ...
      <div className="w-full lg:w-1/2 bg-white flex items-center justify-center p-8 md:p-16">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-3 mb-10">
             ...
             <h1 className="font-bold text-slate-800 text-xl">Touggourt Admin</h1>
          </div>

          <div className="mb-10">
            <h2 className="text-3xl font-bold text-slate-900 mb-2">Welcome Back</h2>
            <p className="text-slate-500">Sign in to manage your tourism database</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-transparent focus:border-orange-200 focus:bg-white rounded-2xl transition-all outline-none text-slate-700"
                  value={email}
            ...
            <div className="flex items-center justify-between py-2">
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="remember" className="text-sm text-slate-500 font-medium">Remember me</label>
              </div>
              <button type="button" className="text-sm text-orange-600 font-bold hover:text-orange-700 transition-colors">Forgot password?</button>
            </div>
          ...
          <p className="mt-8 text-center text-slate-400 text-sm">
            Access restricted to authorized city staff only.
          </p>
        </div>
      </div>
    </div>
